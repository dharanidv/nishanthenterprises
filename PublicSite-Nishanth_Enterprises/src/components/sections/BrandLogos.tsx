...
type BrandLogosProps = {
  /** Full image URLs from CMS (Brands section). Falls back to static logos when omitted or empty. */
  brandImageUrls?: string[];
};

const BrandLogos = ({ brandImageUrls }: BrandLogosProps) => {
  const useCms = brandImageUrls && brandImageUrls.length > 0;
            ...
            ? brandImageUrls.map((logo, index) => (
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      console.error(`Failed to load brand logo: ${logo}`);
                      e.currentTarget.style.display = 'none';
                      const fallback = document.createElement('div');
                      fallback.className =
                        'text-white font-semibold text-xs md:text-sm lg:text-base text-center';
