...
const DEFAULT_SLIDES = [
  ...
  { id: 4, image: '/banner_4.png' },
  { id: 5, image: '/banner_5.png' }
];

type HeroSectionProps = {
  /** Full URLs from CMS (Banners section). Falls back to local banners when omitted or empty. */
  bannerImageUrls?: string[];
};

const HeroSection = ({ bannerImageUrls }: HeroSectionProps) => {
  const [currentSlide, setCurrentSlide] = useState(0);

  const slides = useMemo(() => {
    if (bannerImageUrls?.length) {
      return bannerImageUrls.map((image, i) => ({ id: i + 1, image }));
    }
